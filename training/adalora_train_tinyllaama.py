...
CONFIG = {
    ...
    # Output parameters
    "output_dir": "./adalora_tinyllama",
    "hub_model_id": "safffrron/prompt_tuned_adalora",
    "push_to_hub": True   
}
...
train_size = int(CONFIG["train_split"] * CONFIG["max_examples"])
total_step = ceil(train_size / CONFIG["batch_size"]) * CONFIG["num_epochs"]
...
def train_model():
    ...
    peft_config = AdaLoraConfig(
        ...
        r=8,                         # Initial rank
        lora_alpha=16,
        target_modules=["q_proj", "v_proj"],  # Same as LoRA
        lora_dropout=0.05,
        target_r=4,                  # Final rank after adaptation
        beta1=0.85,                  # Optimizer hyperparameter
        beta2=0.85,
        tinit=200,                   # Warmup steps before rank adaptation
        tfinal=1000,                 # Steps after which rank stabilizes
        deltaT=10,                   # Interval between rank updates
        orth_reg_weight=0.5,         # Encourages orthogonality
